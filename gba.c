...
void drawImageDMAFromColumn(int r, int c, int colOffset, int width, int height, const u16 *image) {
    for (int row = 0; row < height; ++row) {
        DMA[DMA_CHANNEL_3].src = image + OFFSET(row, colOffset, width);
        DMA[DMA_CHANNEL_3].dst = videoBuffer + OFFSET(r+row, c, 240);
        DMA[DMA_CHANNEL_3].cnt = (width-colOffset) | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
    }
}
...
void drawFullScreenImageDMA(const u16 *image) {
  // TODO: IMPLEMENT
    volatile u16* dest = videoBuffer + OFFSET(0, 0, 240);
    DMA[3].src = image;
    DMA[3].dst = dest;
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
    volatile u16* dest = videoBuffer + OFFSET(row, col, 240);

    for (int i = 0; i < height; i++) {
        DMA[3].src = image + OFFSET(i, 0, width);
        DMA[3].dst = dest + OFFSET(i, 0, 240);
...
void drawBackground(const u16 *image) {
    ...
    DMA[DMA_CHANNEL_3].cnt = 38400 | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
}